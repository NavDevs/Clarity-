...
const LoadingStep = ({ text, delay }: { text: string, delay: number }) => {
  const [active, React_useState_active] = React.useState(false);
  const [done, React_useState_done] = React.useState(false);

  React.useEffect(() => {
    const t1 = setTimeout(() => React_useState_active(true), delay * 1000);
    const t2 = setTimeout(() => React_useState_done(true), (delay + 1.5) * 1000);
